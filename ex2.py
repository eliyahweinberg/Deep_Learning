...
# [N=100,H,W,C=1]
# correct labels
y_ = tf.placeholder(tf.float32, shape=[H, N])  # YOUR CODE)

# input data
x = tf.placeholder(tf.float32, shape=[H, D])  # YOUR CODE)
...
num_classes = 2
W_fc1 = weight_variable([D, N])
b_fc1 = bias_variable([1,10])
h_fc1 = tf.nn.relu(tf.matmul(x, W_fc1) + b_fc1) # YOUR CODE

W_fc2 = weight_variable([N, D])
b_fc2 = bias_variable([10,1])
h_fc2 = tf.nn.relu(tf.matmul(h_fc1, W_fc2) + b_fc2) # YOUR CODE
...
for i in range(700):
    ...
    sess.run(train_step, feed_dict={X: input_images, correct_predictions:correct_predictions })# YOUR CODE
    if i % 50 == 0:
